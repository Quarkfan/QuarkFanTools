...
当前版本为 `1.4.1`。应用左下角显示当前版本，点击版本号可查看面向用户的更新记录。项目接续、需求、架构、运维和安全说明统一从 [`docs/AI.md`](docs/AI.md) 开始阅读。当前开发状态见 [`STATUS.md`](STATUS.md)，开发变更历史见 [`CHANGELOG.md`](CHANGELOG.md)。
...
- 每个机器人只能访问明确勾选授权的 Skills；导入或同步的新 Skill 默认不授权给任何机器人。
...
- GUI 提供按机器人启停、独立日志、Skill 授权、连接配置和会话存储管理。
- 消息事件去重，状态和日志保存在本机。
...
导入或同步 Skill 不会自动改变任何机器人的授权。用户态需要针对对应机器人额外完成飞书 OAuth 登录。Bot 态只需要正确配置应用凭据、事件订阅和必要权限。
