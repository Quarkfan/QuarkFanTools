...
test("detects raw lark drive download and export commands", () => {
  ...
  assert.equal(detectRawLarkDriveFileCommand(bashItem("./qft-cli lark drive +export --doc-type slides --file-extension pptx")), "./qft-cli lark drive +export --doc-type slides --file-extension pptx");
  assert.equal(detectRawLarkDriveFileCommand(bashItem("./qft-cli lark drive +download --file-token abc")), "./qft-cli lark drive +download --file-token abc");
});
...
test("allows non-download lark and non-bash commands", () => {
  ...
  assert.equal(detectRawLarkDriveFileCommand(bashItem("./qft-cli lark drive +search --as user")), null);
