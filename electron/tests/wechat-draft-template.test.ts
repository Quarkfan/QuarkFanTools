...
test("wechat draft assistant template parses contact and draft in dry-run mode", () => {
  ...
  assert.match(output.reply, /目标联系人：张三/);
  assert.match(output.reply, /草稿内容：我晚点看一下/);
  assert.match(output.reply, /剪贴板写入：成功/);
  assert.match(output.reply, /微信激活：成功/);
