...
- 全局配置页已将自定义应用产物清理和回复后处理从模型 Provider 面板拆出到 Custom Apps 面板；当前仍是全局默认策略，后续可继续扩展命令级或应用级覆盖。
- 微信未读读取模板点击策略已改为优先 CGEvent 全局点击，并在输出中记录点击策略；新增测试覆盖连续读取时第二个未读候选使用自己的截图坐标和窗口坐标。
