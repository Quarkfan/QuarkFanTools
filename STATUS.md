...
| Message Gateway      | `Message-Gateway/`      | `git@github.com:Quarkfan/Message-Gateway.git`      | 通道配置、可替换飞书后端与持久 Channel Provider 扩展控制面已完成，当前指向 `024491f`。                                                                                                                                |
| Context Hub          | `Context-Hub/`          | `git@github.com:Quarkfan/Context-Hub.git`          | 上下文、检索、记忆治理、projection 边界与持久 Source/Processor 扩展控制面已完成，当前指向 `d7ef0af`。                                                                                                                 |
| Model Hub            | `Model-Hub/`            | `git@github.com:Quarkfan/Model-Hub.git`            | 多类型模型、路由、失败切换、用量与持久 Model Adapter 扩展控制面已完成，当前指向 `7fbd7e2`。                                                                                                                           |
| Capability Registry  | `Capability-Registry/`  | `git@github.com:Quarkfan/Capability-Registry.git`  | 能力注册、导入、绑定、隔离执行、内置能力与持久 Executor 扩展控制面已完成，当前指向 `a0396c8`。                                                                                                                        |
...
| Scheduler Center     | `Scheduler-Center/`     | `git@github.com:Quarkfan/Scheduler-Center.git`     | 调度、立即执行、重试、历史补处理与持久 Trigger/Queue/Dispatcher 扩展控制面已完成，当前指向 `724a2a0`。                                                                                                                |
| Resource Center      | `Resource-Center/`      | `git@github.com:Quarkfan/Resource-Center.git`      | 资源、诊断、清理、FFmpeg 与持久 Storage/Diagnostics/Media 扩展控制面已完成，当前指向 `c08b4cd`。                                                                                                                      |
| Governance Center    | `Governance-Center/`    | `git@github.com:Quarkfan/Governance-Center.git`    | 策略、审批、凭据、脱敏、审计与持久 Policy/Vault/Redactor 扩展控制面已完成，当前指向 `5f49b4a`。                                                                                                                       |
| Platform Console     | `Platform-Console/`     | `git@github.com:Quarkfan/Platform-Console.git`     | Runtime Provider/Profile 和跨中心扩展列表/详情、探针、生命周期、日志、持久代次与时间信息 UI 已完成；生命周期变更仅管理员可用，当前指向 `3ee10f6`。                                                                    |
| Platform Deployment  | `Platform-Deployment/`  | `git@github.com:Quarkfan/Platform-Deployment.git`  | Compose、备份恢复、smoke、E2E、16 页面 UI acceptance、release handoff 与公网 TLS 云边界排查记录已更新，当前指向 `dfbd18e`。                                                                                           |
