...
| Context Hub | `Context-Hub/` | `git@github.com:Quarkfan/Context-Hub.git` | 已迁入 CH 相关设计文档，作为上下文源、知识/RAG、短期/中期/长期记忆、freshness、scope、召回和记忆治理的独立模块。 |
| Reference Projects | `Reference-Projects/` | 父项目目录 | 用于管理 `docs/platform-reference-matrix.md` 中参考项目的本地源码阅读、综合评估和借鉴点抽取；已完成 MG 参考评估，已开始 Context Hub 参考评估，本地 clone 的上游源码放在 `Reference-Projects/sources/` 且不提交。 |
...
- `Context-Hub` 已建立独立模块目录，新增 `AGENTS.md`、`README.md`、`STATUS.md` 和 `docs/context-hub.md`。
- 父项目和 MG / CH / 单机版子项目均有独立接续入口：`AGENTS.md`、`README.md`、`STATUS.md` 或 `docs/AI.md`。
