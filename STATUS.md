...
- Bot 定时任务支持失败重试治理：计划触发失败可按任务配置延迟重试，连续失败超过上限后自动停用并展示暂停原因。
...
- 定时任务当前是本机应用运行期间触发；应用关闭期间不补偿执行，告警和运行历史专页未完成。复杂日历已先通过 5 段 cron 表达式覆盖基础场景，但不支持秒级、`L/W/#` 等高级 cron 扩展。
- 定时任务的用户配置字段和运行状态字段当前仍在同一任务对象中，普通升级安全，但跨 `1.8.x`、`2.0.x` 或未来维护线合并时仍应进一步拆分状态 map，并增加旧配置/新状态交叉合并测试。
...
2. 补定时任务版本合并治理：将任务定义和运行态字段拆成明确 merge helper，增加 `1.8.x` cron 任务、2.0 retry 任务、旧状态文件和回滚后再升级的兼容测试。
3. 增加真实飞书事件、机器人隔离、命令/Workflow/定时任务和会话清理的集成测试。
4. 补充签名、公证和发布自动化。
5. 评估会话过期时间和磁盘配额的用户配置能力。
6. 增强 Skill 市场来源校验、版本展示和更新可见性。
7. 根据分段耗时和排队日志持续评估模型服务延迟、并发数和 Agent turns 上限。
