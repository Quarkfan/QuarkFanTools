# Context Hub First Pass: AnythingLLM / Open WebUI / Dify / LlamaIndex

## Scope

- Center: Context Hub（CH）
- Capability: context source management, knowledge source management, document ingestion, chunk/index model, retrieval contract, permissions, freshness, audit
- Upstream projects:
...
- `Workspace` combines collection-level retrieval settings such as similarity threshold, top N, chat mode, vector search mode, and model selection. For QuarkfanTools, this maps to `ContextCollection` policy, not to runtime session.
...
- `metadata.chunkSource` is used to preserve original source type and path, and resync adapters use it to refetch sources. This maps well to `ContextFreshnessKey` and `ContextSourceRef`.
- Vector DB is resolved through a provider abstraction (`getVectorDbClass`), which supports our principle that vector store choice belongs behind a Context Index adapter.

Borrow carefully:

- AnythingLLM mixes workspace chat product settings with knowledge settings. QuarkfanTools should split `ContextCollection` from runtime/model configuration.
...
- `Knowledge`, `KnowledgeDirectory`, and `KnowledgeFile` separate knowledge base identity, directory organization, and file membership. This is a clean model for `ContextCollection`, `ContextFolder`, and `ContextDocumentBinding`.
...
- `query_knowledge_files` accepts model-attached knowledge, explicit knowledge IDs, or all accessible KBs, then normalizes local vector results, notes, and external knowledge into chunks with source metadata. This maps well to `ContextRetrieveRequest.sources` and mixed source retrieval.
...
- Its RAG injection lives close to chat middleware. QuarkfanTools should keep retrieval result generation in CH and prompt construction in Runtime Center.
- Notes, model-attached knowledge, and files share a convenient product UX, but our center model should keep `ContextSource` and `ResourceRef` distinct.
- Do not copy the full Open WebUI app/tool surface into CH.
...
- `KnowledgeConfig` explicitly models data source, process rule, retrieval model, summary index setting, doc form, embedding model, and multimodal flag. This is the strongest reference for our `ContextIngestionConfig` and `ContextIndexConfig`.
...
- `InnerKnowledgeRetrieveRequest` is a useful upstream internal-contract reference: caller context, dataset IDs, query or attachments, retrieval strategy, metadata filtering, validation, and usage reporting. In QuarkfanTools this should become `ContextRetrieveRequest`, not leak Dify's naming.
...
- Dify is a full app/workflow platform. QuarkfanTools should not put workflow execution or app orchestration into CH.
...
- `NodeWithScore` cleanly couples a retrievable node with a score while leaving content and metadata on the node. This maps to `ContextRetrieveResult.records[].score`.
...
- LlamaIndex is a broad library ecosystem, not an application boundary. It is best as a model/interface reference or optional adapter, not as the whole CH.
...
interface ContextSourceAdapter {
  ...
  getFreshness(request: ContextFreshnessRequest): Promise<ContextFreshnessKey>;
}
...
2. Treat Open WebUI as the best reference for knowledge permissions, knowledge-directory/file membership, and tool-like retrieval. Keep RAG injection outside CH.
3. Treat Dify as the best reference for ingestion/retrieval configuration and internal retrieval DTOs. Keep its workflow and tenant model out of CH.
...
5. Keep CH independent from Resource Center: CH stores semantic facts, memory records and indexes; Resource Center owns raw files, caches, materialized workspace files, logs, and diagnostics bundles.
6. Keep CH independent from Runtime Center: it returns retrieval results and memory candidates; Runtime decides how to fit them into the current runtime prompt/context.
...
1. Contract doc for CH DTOs and interfaces.
2. Read-only adapter for existing Skill `knowledge/`.
3. Read-only adapter for existing controlled Lark file/doc cache refs.
4. Local JSONL/SQLite metadata store for `ContextSource`, `ContextRecord`, `ContextChunk`, `ContextMemory`, freshness and audit.
5. Basic keyword retrieval over text chunks.
6. Optional vector index adapter behind `ContextIndex`, not directly in runtime.
...
- How much of CH P0 should live inside `QuarkfanTools-Single` before becoming a separate submodule?
- Should context freshness be strict by default, or allow stale-but-marked results when a source is temporarily unavailable?
