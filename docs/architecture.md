...
飞书事件 WebSocket 连接地址由 `lark-cli` 依赖的 `github.com/larksuite/oapi-sdk-go/v3/ws` 向飞书服务端获取，SDK 再按服务端返回的 endpoint URL 建连。官方 `lark-cli event +subscribe --force` 帮助提示多个订阅会被服务端随机拆分事件，现场也确认一个 Bot 的连接可能收到另一个 Bot 被 @ 的消息。因此飞书 Provider 在同一应用进程内只保留一个共享 `LarkEventStream` 入口，收到 NDJSON 事件后按 mention 目标路由到唯一目标 Bot；回复、表情、附件下载和 Agent 内 lark-cli 调用仍使用目标 Bot 的独立 named profile。企业微信事件桥和后续钉钉 Provider 保持各自 Provider 边界，不复用飞书共享入口。
