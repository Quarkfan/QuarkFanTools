...
微信桌面辅助 Agent PoC 不新增微信或个人微信 IM Provider，也不恢复企业微信 Provider。它通过自定义应用 manifest 的 `permissions.desktopAutomation` 声明屏幕录制、辅助功能、剪贴板、键盘输入和自动发送意图；能力治理会把这些权限作为高风险项展示。当前 `electron/desktop-agent.ts` 生成和校验动作计划，包括激活微信、读取可见未读、搜索联系人、截图校验、粘贴草稿和等待用户确认；内置模板会通过 AppleScript 激活微信，使用 System Events 读取微信窗口边界并截取当前微信窗口，先做本地初筛，再通过 `visionRequest` 交给主进程使用已配置的多模态模型识别截图中的可见未读。自定义应用 runner 支持带状态的多轮 `visionContinuation`：模板会把视觉模型返回的可见未读候选保存为本轮队列，按顺序点击最多 5 个可见会话，每打开一个会话就再次截图并让多模态模型抽取当前窗口可见消息；同一会话多条消息只覆盖当前窗口可见范围，不读取不可见历史。点击会话可能让微信把该会话标记为已读。模型 API Key 只留在主进程配置中，不传给自定义应用脚本。模板可选使用 `pbcopy` 写入草稿剪贴板，但不会读取微信数据库、协议或进程内存，也不会自动搜索联系人、自动粘贴或发送。`autoSend=true` 当前作为阻断诊断处理，避免 PoC 绕过人工确认。自定义应用最终回复可按 `runtime.customAppReplyProcessing` 配置原样返回或由主进程调用当前模型做一次无工具总结。
