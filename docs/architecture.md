...
QuarkfanTools 是 Electron 桌面应用。主进程负责配置、Skill 管理、存储管理、Bot worker 进程监督、日志和状态聚合；渲染进程只通过 IPC 使用受控能力。`v1.7.0` 起，每个运行中的 Bot 由独立 worker 进程承载，worker 内部负责该 Bot 的飞书监听、会话编排、Claude Agent 调用和本地状态。

```mermaid
flowchart LR
    UI["Electron GUI"] -->|IPC| Main["Electron Main"]
    Main --> Supervisor["Bot Supervisor"]
    Supervisor --> WorkerA["Bot Worker A"]
    Supervisor --> WorkerB["Bot Worker B"]
    WorkerA --> Lark["Bundled lark-cli"]
    WorkerA --> Skills["Skill Resolver"]
    WorkerA --> Sessions["Session Store"]
    WorkerA --> Office["Office Preprocessor"]
    WorkerA --> Claude["Claude Agent SDK"]
    Claude --> Lark
...
- 每个运行中的机器人拥有独立 worker 进程；主进程只管理 worker 生命周期和聚合状态。
...
- 配置中的 `runtime.botIsolationMode` 默认是 `process`。`container` 和 `auto` 作为 Docker driver 的预留模式进入配置和诊断，默认交付仍不要求用户安装 Docker。
