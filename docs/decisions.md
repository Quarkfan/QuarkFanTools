...
- 状态：已采用
- 决策：版本号使用 `主版本.次版本.修订版本`。大的应用能力或不兼容变化升级主版本；新增明显完整功能升级次版本；修复、优化和轻量小能力升级修订版本。根 `CHANGELOG.md` 面向开发与发布，应用内 `electron/release-notes.ts` 面向用户。后续形成具体版本号后必须在同一轮完成 arm64 打包验证；未打包变化只保留在 `Unreleased`。Intel x64 不再作为当前发布目标。
- 原因：让版本变化能够表达影响范围，同时让普通用户在应用内直接理解更新内容。

## D-011 升级兼容优先且不可兼容前必须备份

- 状态：已采用
- 决策：旧版本升级到 2.0 后，配置、Bot 状态、会话、定时任务运行态、用户导入 Skills、自定义应用、套件和文件缓存应优先通过兼容读取或迁移适配。确实无法无损兼容的变更，必须先在应用数据目录下写入本地备份，再执行迁移或重建。备份不得包含额外上传行为，也不得绕过现有敏感信息本机存储边界。
- 原因：2.0 的能力治理、Bot 隔离和本机状态目录变化较大，升级失败时必须保留人工恢复入口，不能让迁移过程成为不可逆破坏。
