...
当前版本 `1.8.1`。当前状态、风险和下一步见 [`../STATUS.md`](../STATUS.md)。
...
| `src/main.ts` | GUI 渲染层 |
| `electron/runtime-supervisor.ts` | 主进程 Bot worker 监督、日志与状态聚合 |
| `electron/bot-worker.ts` | 单 Bot worker 入口 |
| `electron/runtime.ts` | worker 内单 Bot 监听、任务排队与消息处理总编排 |
| `electron/scheduled-tasks.ts` | Bot 维度定时任务存储、归一化和 worker 调度器 |
