...
LiteLLM 是最适合的主参考。它是开源 AI Gateway，提供统一接口调用 100+ LLM provider，并支持 spend tracking、guardrails、load balancing、cost tracking、project/user 级 spend management、virtual keys、retry/fallback 等能力。([GitHub](https://github.com/BerriAI/litellm?utm_source=chatgpt.com))
...
|P2|token / cost 统计|
|P2|健康检查|
|P3|local model / self-hosted model|
...
|**顺序**|**中心**|**原因**|
|---|---|---|
|4|模型中心|provider、fallback、usage、cost 可以独立沉淀|
