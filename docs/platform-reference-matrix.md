...
3. Model Hub（MH，模型枢纽）
4. Capability Registry（CR，能力注册中心）
5. 运行时中心
6. 资源中心
7. 系统与编排中心
8. 治理与安全中心
...
|Capability Registry（CR）|MCP|Dify Plugin、Open WebUI Pipelines、LangChain Tools、Composio、n8n、Pipedream、VS Code Extension Model|工具协议、能力声明、插件、外部系统连接、扩展 manifest|发现即信任、直接暴露全部工具、把 registry 做成 executor|
...
|P2|token / cost 统计|
|P2|健康检查|
|P2|ModelCapabilityExport 给 CR 登记模型能力|
|P3|local model / self-hosted model / diffusion serving|

---

# **7. Capability Registry（CR）参考规划**

## **7.1 中心定位**

Capability Registry（CR，能力注册中心）负责“平台有什么能力、如何声明、如何解析绑定”，治理中心负责“某场景能不能用”。
...
|外部 SaaS 集成|Composio / n8n / Pipedream|app integration、auth、tool catalog、trigger/action|ExternalCapabilityConnector|
|扩展 manifest|VS Code Extension Model|extension manifest、contribution points、activation events|CapabilityPackage|
...
```mermaid
flowchart LR
    RT[运行时中心] --> CAP[Capability Registry]
    CAP --> GOV[治理与安全中心]
    GOV --> CAP
    CAP --> RT
    RT --> RES[资源中心]
...
|5|Capability Registry|Skill、MCP、自定义应用、Workflow 需要统一能力声明|
...
|Capability Registry|CapabilityManifest、CapabilityRegistry、CapabilityBinding|
