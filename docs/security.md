...
- Manifest 可声明 `desktopAutomation` 权限元数据。屏幕录制、辅助功能、剪贴板和键盘输入都属于高风险本机能力，必须通过能力治理提示和 Owner 审批控制；当前 PoC 阶段 `autoSend=true` 直接阻断导入或升级。
...
自动发送风险高于生成草稿。当前 PoC 只允许把草稿写入输入框并停止；后续若评估自动发送，必须至少满足联系人白名单、会话标题一致、识别置信度达标、低风险内容、发送前截图存档和用户预批准规则。涉及金额、合同、报价、承诺、投诉、敏感内部信息的内容不得自动发送。
