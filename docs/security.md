...
读取和自动发送都必须限定在用户可见的微信客户端内。当前 PoC 最多尝试截取微信当前窗口，并通过本地初筛和已配置多模态模型识别可见未读，再可选把草稿写入系统剪贴板；不自动搜索、粘贴或发送。截图只用于用户当前主动触发的命令，不读取微信数据库、协议或进程内存。后续若评估自动发送，必须至少满足联系人白名单、会话标题一致、识别置信度达标、低风险内容、发送前截图存档和用户预批准规则。涉及金额、合同、报价、承诺、投诉、敏感内部信息的内容不得自动发送。
