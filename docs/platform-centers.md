...
- 负责知识托管、知识源接入、知识目录、索引、召回和权限范围。
- 对外提供知识召回能力；内部可以逐步演进为 RAG、知识图谱、向量索引、文档快照、外部知识连接器等。
- 统一管理 Skill 自带 `knowledge/`、飞书文档/Wiki/云盘、未来本地知识库和客户知识库。
...
- 目前知识能力分散在 Skill、飞书连接器、受控文件缓存和 prompt 约束中。
- `electron/platform-connectors.ts`
...
- 新增 `KnowledgeProvider` 抽象。
- 把“召回什么知识”和“运行时怎么使用这些知识”分开。
- 将飞书文档、Skill knowledge 和未来本地 RAG 统一成可审计召回结果。
...
Skill 可以带 `knowledge/`，飞书连接器可以读云文档，自定义应用也可能产出知识，但知识中心的对外职责是“召回知识”。工具中心的职责是“管理可执行或可注入能力”。未来 RAG 和知识图谱应优先归入知识中心，而不是继续散在 prompt 或 Skill 目录约定里。
...
- 新增 `KnowledgeProvider` / `KnowledgeRetriever` 抽象。
- 让飞书文档、Skill knowledge、受控文件缓存和未来本地 RAG 都通过统一召回结果进入 Agent 上下文。
- 召回结果必须包含来源、Bot 授权、更新时间或 freshness key、可审计摘要。
