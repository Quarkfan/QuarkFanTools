...
- 对运行时中心、CH、CR 提供可用模型候选、attempt plan 或 `ModelCapabilityExport`，不直接等同于 Agent runtime。

不负责：

- 不决定工具权限。
- 不注册、展示或编排工具；CR 可以登记 MH 导出的模型能力。
...
### 2.4 Capability Registry（CR，能力注册中心）

职责：

- 统一登记 Skill、MCP、套件、自定义应用、Workflow、命令映射、外部连接器，以及由 MH / CH 导出的可封装能力。
- 提供能力发现、展示、检索、诊断、导入、升级、卸载、package 生命周期和能力声明。
...
Stable Diffusion / TTS / OCR 等模型能力可以由 CR 登记为可绑定能力，但不自动变成 runtime。

### 3.2 CR 不等于治理中心

CR 负责“有什么能力、如何声明、如何绑定”。治理中心负责“谁在什么场景能不能用”。能力导入、发现、展示和诊断不应自动扩大 Bot 授权；运行时执行前必须经过治理判定。

### 3.3 CH 不等于 CR

Skill 可以带 `knowledge/`，飞书连接器可以读云文档，自定义应用也可能产出知识或记忆候选，但 CH 的对外职责是“召回上下文和治理记忆”。CR 的职责是“登记、发现、解析、绑定和诊断能力”。未来 RAG、知识图谱和记忆管理应优先归入 CH，而不是继续散在 prompt、Skill 目录约定或 runtime transcript 里。
