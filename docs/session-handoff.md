...
- 最近一次 UI 发布前验证备份：`/opt/quarkfantools/Platform-Deployment/backups/20260816T075300Z`。
...
| Platform Console | `385b44b`（生产应用代码 `f08e9db`） |
| Platform Deployment | `e3449dc` |
...
- 最新候选在行内只保留健康标签；最后检查时间、可用延迟和最近错误通过标签悬停浮窗显示，鼠标移出后隐藏。
...
- Deployment UI acceptance 已验证检查详情存在于健康标签悬停提示中，且不常驻在列表行内；所有 32 个桌面/移动页面状态通过。
...
- 父项目源码 `d8020a9`、Console 应用 `f08e9db` 已部署；备份 `20260816T075300Z`、完整 E2E、回环认证、32 状态 UI 验收与最终 12 服务 smoke 全部通过。
