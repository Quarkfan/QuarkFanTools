...
- 自动化页按 Bot 管理定时任务。任务文件保存在 `state/bots/<bot-id>/scheduled-tasks.json`。
- 支持“间隔”“一次性”和“Cron 表达式”触发；Cron 使用 5 段 `分钟 小时 日 月 周`，支持 `*`、列表、范围和步进，例如 `15 9 * * 1-5` 表示工作日 09:15。任务内容为 prompt；结果只写运行台日志，不主动发送飞书消息。
- 手动运行要求目标 Bot 已启动，因为任务由对应 Bot worker 执行。
- 如果任务没有执行，先确认目标 Bot 是否正在运行，再查看该 Bot 日志中的“Bot 定时任务调度器已启动”“正在运行定时任务”和“定时任务执行失败”。
- 当前错过策略和并发策略已经进入配置结构；后续再扩展飞书输出目标、任务会话重置和更细粒度 missed policy。

## 6. 诊断日志

- 运行台点击“复制日志”可复制诊断日志。
- 诊断日志包含当前运行快照、Bot 状态路径、订阅 PID、每个 Bot 的 lark-cli 日志尾部、最近内存日志和 `state/logs/quarkfantools.jsonl` 中最近的持久化日志。
- 诊断日志会隐藏模型 API Key 和飞书 App Secret，只保留是否已配置、Bot ID、Bot 名称、运行状态、事件类型和日志内容。
...
- 检查是否已有另一个运行中的本地 Bot 使用相同 App ID。同一飞书应用同一时间只能对应一个本地 Bot；多角色应放在同一个 Bot 下通过 Skill、命令或套件区分。
- 检查 Claude 兼容模型的 Base URL、模型名和 API Key 是否完整。
- 检查日志中是否存在旧监听进程或飞书 CLI 错误。
- `1.6.15` 起，点击运行台“启动”会立即写入本地启动日志，并记录“收到机器人启动请求”“正在确认飞书 Bot 身份”等阶段。如果仍没有任何新日志，优先检查前端事件或应用是否为最新版本；如果停在飞书 CLI 配置、初始化或密钥降级阶段，短命令 30 秒后会显示明确超时错误。
...
- 应用应使用推荐权限发起 OAuth。
- `1.6.14` 起，用户态 OAuth 存在每个 Bot 专属 HOME 下，不再读取旧的 macOS 用户全局 lark-cli 授权。升级后需要在每个需要读取飞书资料的 Bot 配置页重新点击用户态授权。
...
- 如果回复提到 `master.key.file`、授权令牌或 lark-cli 安全存储被 sandbox 阻止，确认正在运行 `1.6.14` 或更高版本，并在对应 Bot 配置页重新完成用户态 OAuth；新版本只应访问 `state/bots/<bot-id>/lark-home/Library/Application Support/lark-cli/`。
...
- 当前仅支持这种用户确认后的延后任务，不支持把单个延后下载任务预约到任意指定时间；Bot 自动化页的 Cron 定时任务是另一条调度能力。
