...
任务编辑弹窗中的“治理”区可配置失败重试。最大重试次数为 `0` 时不立即重试；大于 `0` 时，计划触发失败会按“重试延迟分钟”设置下一次 `retryAt`，并优先于原计划触发。连续失败超过上限后任务会自动停用，并在列表和任务编辑弹窗显示暂停原因。手动立即执行不消耗重试次数；手动或计划执行成功会清空连续失败状态。
...
文件缓存位于 `state/file-cache/<sha256>/`，用于复用飞书下载的大文件和 Agent 生成文件。缓存按内容哈希去重，metadata 记录关联 Bot。用户可在存储管理中单独清理全部文件缓存；清理后不删除会话记录，但后续需要相关文件时可能重新下载或生成。
...
存储管理会展示文件缓存索引。索引行展示来源类型、关联 Bot、文件名、大小和来源摘要，可按 Bot 或来源类型筛选，并支持删除单条缓存索引。单条删除只接收索引 `cacheKey`，不会暴露全局缓存目录路径；如果同一内容 hash 仍被其他索引引用，内容目录会保留到最后一个引用被删除。
...
- 如果任务自动停用，打开 Bot 编辑弹窗查看任务摘要或任务编辑弹窗中的“暂停原因”。确认问题修复后重新启用并保存任务，会清空失败计数、重试时间和暂停原因。
