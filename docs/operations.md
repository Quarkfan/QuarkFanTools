...
配置页的 Custom Apps 面板提供“自定义应用回复后处理”，默认原样返回自定义应用输出；选择“交给大模型总结后返回”时，主进程会使用当前模型配置对最终回复做一次无工具文本总结。该总结不会把 API Key 传给自定义应用脚本。适合把微信多会话读取结果整理为更短的用户回复；如果需要完整原始证据，应保持原样返回。当前这是自定义应用的全局默认策略，不属于模型 Provider 配置；后续需要时可继续扩展为命令级或应用级覆盖。
