...
导入或同步后，需要进入机器人配置明确授权 Skills。新增 Skill 不会自动进入任何机器人的权限范围；Skill 较多时可搜索名称或描述，并对当前筛选结果批量授权或取消。
...
- 若接收及时但处理慢，查看“消息处理耗时”日志中的资源、Agent、飞书回复和总计。Agent 段长通常表示模型服务响应慢、Skill 工具调用较多或达到最大 turns；飞书回复段长表示回复 API 或 CLI 慢。
