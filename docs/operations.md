...
配置页的 Custom Apps 面板按自定义应用分别提供“回复后处理”。每个应用默认原样返回输出；选择“交给大模型总结后返回”时，主进程会使用当前模型配置对该应用最终回复做一次无工具文本总结。该总结不会把 API Key 传给自定义应用脚本。适合只让微信多会话读取这类高噪声应用整理为更短回复，同时让其他应用保留完整原始输出。旧版全局 `runtime.customAppReplyProcessing` 只作为兼容兜底，新配置应写入 `runtime.customAppReplyProcessingByApp[appId]`。
